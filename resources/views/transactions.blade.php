...
<body>
    ...
    <div id="searchbar" class="w-full py-4 px-20 gap-4 flex flex-row justify-center items-center">
        <input type="text" id="" oninput="handleSearch(this.value)" placeholder="Search Transaction by email"
            class="w-full border-2 border-black rounded-xl outline-none focus:border-blue-600 px-5 py-2">
    </div>
    <br>
        ...
        <tbody id="result">
            ...
            @foreach ($transactions as $key => $value)
                <tr class="hover:bg-blue-100">
                    ...
                    <td class="px-5 py-2 text-left">{{ $value["email"] }}</td>
                    <td class="px-5 py-2 font-semibold">{{ $value["total"] }}</td>
                </tr>
            @endforeach
        </tbody>
    </table>
    <script>
        ...
        function handleSearch(value) {
            ...
            transactions.forEach(value => {
                if (value["email"].toLowerCase().includes(text)) {
                    result.push(value);
                }
            });
